...
void main() {
	...
	int opcao = 0, i = 0, totalEstudantes = 0, totalMatriculas = 0, id = 0, error = 0, confirmar = 0, submenu = 0, aux1 = 0, whati = 0, SimOuNao = 0, auxmat = 0, x = 0, have = 0, list = 0, listaOpcao = 0, go = 0, j = 0;
	float auxnota1 = 0, auxnota2 = 0, auxaverage = 0;
	turma student[100];
	system("color b9");
		...
		if (opcao == 1) 
		{
			...
			while(submenu != SUBMENU) 
			{
				...
				while(confirmar != CONFIRM) 
				{
					...
					student[totalMatriculas].mediatotal = (student[totalMatriculas].nota1 + student[totalMatriculas].nota2) / 2;
					gotoxy(5,9);
					printf("Media: %.2f\n\n", student[totalMatriculas].mediatotal);
					gotoxy(5,12);
		...
		if(opcao == 3) 
		{
			...
			else 
			{
				for(i = 0; i < totalMatriculas; i++)
				{
					...
					{
						if(student[i].mediatotal < student[x].mediatotal)
						{
				...
				while(list != LIST) 
				{
					...
					{
						if (student[j].mediatotal != 1999 && j != totalMatriculas) 
						{
			...
			while (submenu != SUBMENU) 
			{
				...
				if (id != 98) 
				{
					...
					if (aux1 == 1) 
					{
						...
						student[whati].mediatotal = (student [whati].nota1 + student[whati].nota2) / 2;
						gotoxy(5,12);
						printf("Media: %.2f\n", student[whati].mediatotal);
						gotoxy(0,17);
						system("pause");
						system("cls");
						submenu = 2;
